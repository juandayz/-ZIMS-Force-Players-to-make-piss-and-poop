private ["_thirstPiss","_hungerPoop","_timerBasicsNeeds","_PissTimer","_PooTimer","_notifiedPiss","_notifiedPoo","_hunger","_thirst","_timeOut","_result","_randomSpot","_distance","_mylastPos","_lastTemp","_rnd","_messTimer","_PlayerNearby","_ZedsNearby","_saveTime"];
...
_timerBasicsNeeds = diag_ticktime;
_notifiedPiss = false;
_notifiedPoo = false;
_PissTimer = 0;
_PooTimer = 0;
...
//Thirst
    ...
    _thirstPiss = round(100 - (dayz_thirst / SleepWater) * 100);
    _hungerPoop = round(100 - (dayz_hunger / SleepFood) * 100);
 
    if (([0.15] call fn_chance) && (player getvariable ["needpoo",0] !=1) && (_hungerPoop >=60)) then {
    if (!_notifiedPoo) then {systemchat "I have a terrible pain in my stomach...I need to find a tree to poop.";_notifiedPoo = true;};
    player setvariable ["needpoo",1,true];
    call player_BasicNeeds;
    _PooTimer = _PooTimer +1;  
    }else{
    _notifiedPoo = false;
    _PooTimer = 0;
    };
   
    if (([0.75] call fn_chance) && (player getvariable ["needpiss",0] !=1) && (_thirstPiss >=60)) then {
    if (!_notifiedPiss) then {systemchat "I can not take it anymore.I'm pissing myself...Need a tree!.";_notifiedPiss = true;};
   
    player setvariable ["needpiss",1,true];
    call player_BasicNeeds;
    _PissTimer = _PissTimer +1;
    }else{
    _notifiedPiss = false;
    _PissTimer = 0;
    };
